			...
			else
			{
				...
				for(i=0;i<3;i++)
				{
					...
					queue_wireless_send.len = FrameRouterCompose_ext(joine_cmd->mac, 
							...
							0,
							0);
					xTaskNotifyWait(0x00000000, 0xFFFFFFFF, &ulValue, 2);	//等待回应
					
					queue_wireless_send.toCh = Default_Channel;
					xQueueSend(xQueueWirelessTask, &queue_wireless_send, (TickType_t)10);
											
					xResult = xTaskNotifyWait(0x00000000, 0xFFFFFFFF, &ulValue, 2500);	//等待回应
				...
				{   
					...
					for(i= 0;i<deviceNum;i++)
					{
						if(deviceBuff[i] == userFrame->addr_DA)continue;
						routerTab[0]= deviceBuff[i];
...
void AppTaskCreate(void)
{
    ...
    osThreadDef(NetCreateTask, AppNetCreateTask, osPriorityNormal, 6, 256);
    ...
    osThreadDef(WirelessTask, AppWirelessTask, osPriorityNormal, 3, 256);
	...
	if(NetCreateTaskHandle == NULL) printf("NetCreateTask Create Error") ;
	
    UartTaskHandle = osThreadCreate(osThread(UartTask), NULL);
	if(UartTaskHandle == NULL) printf("UartTask Create Error") ;
	
    LedTaskHandle = osThreadCreate(osThread(LedTask), NULL);
	if(LedTaskHandle == NULL) printf("LedTask Create Error") ;
	
    TouchTaskHandle = osThreadCreate(osThread(TouchTask), NULL);
	if(TouchTaskHandle == NULL) printf("TouchTask Create Error") ;
	
    WirelessTaskHandle = osThreadCreate(osThread(WirelessTask), NULL);
	if(WirelessTaskHandle == NULL) printf("WirelessTask Create Error") ;
	
    SuperviseTaskHandle = osThreadCreate(osThread(SuperviseTask), NULL);
	if(SuperviseTaskHandle == NULL) printf("SuperviseTask Create Error") ;
	
    RetryTaskHandle = osThreadCreate(osThread(RetryTask), NULL);
	if(RetryTaskHandle == NULL) printf("RetryTask Create Error") ;
