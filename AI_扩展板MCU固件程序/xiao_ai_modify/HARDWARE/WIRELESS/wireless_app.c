...
{
    uint32_t Time;
    srand(SysTick->VAL);       					//利用滴答定时器值产生随机数种子
    Time = ((uint32_t)rand() % (b - a + 1)) + a;         //产生一个a-b范围的随机数
    Si4438_Delay_ms(Time);        //延迟
}
...
{
    uint8_t int_status[8] = {0};
//  HAL_GPIO_WritePin(SI4438_TX_GPIO_Port, SI4438_TX_Pin, GPIO_PIN_RESET);
	SI4438_PA_TX_L();
//  HAL_GPIO_WritePin(SI4438_RX_GPIO_Port, SI4438_RX_Pin, GPIO_PIN_SET);
	SI4438_PA_RX_H();
    Get_INTState(int_status, 0, 0, 0);
...
Wireless_ErrorType Si4438_Transmit_Start(WLS *pWL, uint8_t channel_number, uint8_t *addr, uint8_t length)
{
    ...
    pWL->Wireless_FrameHeader[3] = (pWL->Wireless_FrameHeader[0] ^ pWL->Wireless_FrameHeader[1]) ^ pWL->Wireless_FrameHeader[2];
    pWL->Channel_Busy_cnt = 0; //clear busy time
//  HAL_GPIO_WritePin(SI4438_TX_GPIO_Port, SI4438_TX_Pin, GPIO_PIN_SET);
	SI4438_PA_TX_H();
//  HAL_GPIO_WritePin(SI4438_RX_GPIO_Port, SI4438_RX_Pin, GPIO_PIN_RESET);
	SI4438_PA_RX_L();
...
uint8_t PLME_CCA(void)
{
    ...
    {
        Read_FRR();
        if ((Si4438Cmd_Reply.FRR_READ.FRR_B_VALUE & 0x08) == 0x08)
            return CHANNEL_BUSY;
    }
    return CHANNEL_FREE;
}
...
Wireless_Handle_Status si4438_DataReceive(WLS *pWL)
{
    ...
    */
    ...
    if (!(ph_status & 0x19))
        return RECEIVE_NO_DATA;
    if (((ph_status & 0x08) == 0x08) || ((ph_status & 0x11) == 0x11))
        return RX_ERROR;
    ...
    while (((ph_status & 0x01) == 0x01) && (RX_REMAIN > RX_THRESHOLD)) //剩余数大于门槛
    {
        ...
        do
        {
            if (++RX_timeout > (uint8_t)(RX_THRESHOLD * 8 / 10 + 10))
                return RECEIVE_TIMEOUT; //超时检测,由于有误差,附加10ms
            Si4438_Delay_ms(1);
...
Wireless_Handle_Status si4438_DataSend(WLS *pWL, uint8_t channel_number, uint8_t *addr, uint8_t length)
{
    ...
    if ((length > 251) || (channel_number > 65))
        return SEND_PARA_ERROR;
    ...
    do
    {
        if (++TX_timeout > (uint8_t)(TX_THRESHOLD * 8 / 10 + 10))
            return SEND_TIMEOUT; //超时检测,由于有误差,附加10ms
        Si4438_Delay_ms(1);
        Get_PHstate(ph_state);
        ph_status = ph_state[0];
        //Read_FRR();
