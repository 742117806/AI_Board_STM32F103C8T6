...
void GetRout_FromLogicTab(HKPath_Try_TypDef *p_ps, uint8_t *pp)
{
  ...
  while(1)  
  {
    ...
    {
      if((p_ps->Current_Try_Number1 < node_number) && (p_ps->Current_Try_Number1 != p_ps->Current_Try_Number2) \
        && (p_ps->LogicAddr_Tab[p_ps->Current_Try_Number1] != p_ps->Distance_Addr) && (p_ps->LogicAddr_Tab[p_ps->Current_Try_Number2] != p_ps->Distance_Addr))
        break;
    }
